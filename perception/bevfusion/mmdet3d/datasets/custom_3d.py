...
class Custom3DDataset(Dataset):
    ...
    def prepare_train_data(self, index):
        ...
        single_pred_origin = input_dict["single_pred"] 
        ...
        for i in range(len(single_pred_origin)):
            box_list = torch.tensor(single_pred_origin[i]) 
            if box_list.shape[0] <= 0:
                continue
            confidence = box_list[:, -1].reshape(-1, 1) # [N, 1]
            conf_padding = torch.full((confidence.shape[0], 1), i) 
        ...
        single_pred_after = all_boxes[divider_indice[0]+1:, 0:7] 
